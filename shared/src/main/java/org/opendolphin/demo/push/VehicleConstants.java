...
class VehicleConstants {
    ...
    static final List<String> ALL_ATTRIBUTES = Arrays.asList(ATT_X, ATT_Y, ATT_WIDTH, ATT_HEIGHT, ATT_ROTATE, ATT_COLOR);

    static String unique(String part) {
        return VehicleConstants.class.getName() + "-" + part;
    }

    static String qualify(String id, String attributeName) {
        return unique(id + '.' + attributeName);
    }
}
